...
class DataForm extends Widget
{
	...
	public function setSource($source)
	{
		...
		if (is_object($source) and (is_a($source, '\\Rapyd\\Model')))
		{
			$this->model = $source;
		} elseif (is_string($source))
		{
			//$this->model =  \Rapyd\Model();
            //trigger error ?
		...
		if (count($this->fields))
		{
			...
			{
				if (in_array($field_obj->name, $this->app->db->table($table)->columns))
				{
					$field_obj->model = $this->model;
				}
			}
