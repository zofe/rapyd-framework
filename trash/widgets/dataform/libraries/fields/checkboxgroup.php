    ...
    switch ($this->status)
    {
      ...
      case "modify":
        ...
        if (count($this->option_groups))
        {
            ...
            foreach ($this->option_groups as $group_id => $group )
            {
                ...
                foreach ($group['options'] as $val => $label )
                {
                  ...
                  $group_output .= sprintf($this->format, form_helper::checkbox($attributes, $val , $this->checked).$label).$this->separator;
                }

                $output .= sprintf($this->format_group, $group['label'], html_helper::image('checked.png',array('class'=>'group_check')).' '.html_helper::image('unchecked.png',array('class'=>'group_uncheck')) ,$group_output). $this->extra_output();
        ...
        foreach ($this->options as $val => $label )
        {
          ...
          $attributes['id'] = $this->name.'_'.$i++;
          $this->checked = in_array($val,$this->values);
          $output .= sprintf($this->format, form_helper::checkbox($attributes, $val , $this->checked).$label).$this->separator;
        }
